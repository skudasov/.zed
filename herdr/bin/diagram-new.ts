 ...
 * the repo you were looking at rather than somewhere central.
 *
...
// Only when it is not there: relaunching on an existing diagram should reopen
// it, never overwrite the work.
if (!(await Bun.file(source).exists())) await Bun.write(source, STARTER);
...
const preview = browser
  ...
  : // timg on a d2 render, redrawn on save. --svg keeps diagram.svg beside the
    // source as well, so the workbench still leaves something to share.
    { command: `${PREVIEW} --watch --svg ${SOURCE}`, note: `${join(dir, OUTPUT)} kept in step` };
