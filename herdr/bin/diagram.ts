 ...
 *   diagram.ts arch.d2 --watch redraw in THIS pane on every save (--svg keeps arch.svg too)
...
async function render(cols: number, maxRows: number): Promise<{ image: string; engine: string }> {
  ...
  const tried = await Promise.all(
    LAYOUTS.map(async (engine) => {
      ...
      const built = await $`d2 --layout ${engine} --theme ${THEME} --pad 20 ${source} ${out}`.quiet().nothrow();
      const size = built.exitCode === 0 ? await pngSize(out) : undefined;
      return { engine, out, size, error: built.stderr.toString().trim() };
    }),
  );
...
if (flag("watch")) {
  ...
  for (;;) {
    ...
    if (now !== last || redraw) {
      ...
      try {
        const { image, engine } = await render(cols, rows);
        // The picture on screen is a PNG; --svg also leaves a scalable copy next
        // to the source, drawn the same way, for anything outside the terminal.
        if (flag("svg") && kind === ".d2") {
          const svg = source.replace(/\.d2$/, ".svg");
          await $`d2 --layout ${engine} --theme ${THEME} --pad 20 ${source} ${svg}`.quiet().nothrow();
        }
        await $`sh -c ${timg(image)}`.nothrow();
      } catch (error) {
        console.clear();
